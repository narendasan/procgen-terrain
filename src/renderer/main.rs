...
fn main() {
    ...
    let mut previous_frame_end: Option<Box<dyn GpuFuture>> = Some(Box::new(sync::now(renderer.device.clone())));

    let terrain = Terrain::new((512, 512), 42);
    let mut terrain_model = terrain.as_model((128.0, 128.0), 256);

    renderer.set_view(&look_at(&vec3(0.0, 0.3, 1.0), &vec3(0.0, 0.0, 0.0), &vec3(0.0, -1.0, 0.0)));
    ...
    events_loop.run(move |event, _, control_flow| {
        match event {
            ...
            Event::RedrawEventsCleared => {
                ...
                renderer.start();
                renderer.geometry(&mut terrain_model);
...
//     let mut model = Model::new("data/models/tree.obj").build();
